...
let containedHandler message (self : NamedObject) loop (state : ThingState) = 
    actor { 
        match message with
        | SetContainerByActorRef(newContainer) -> newContainer <! AddContent(self)
...
let notifyHandler message (self : NamedObject) loop (state : ThingState) = 
    actor { 
        match message with
...
let containerHandler message (self : NamedObject) loop (state : ThingState) = 
    actor { 
        match message with
        ...
        | ExitContainer(who, _) -> state.container.ref <! EnterRoom(who, self)
        return! loop state
    }

//handles messages for agents that can interact with the world
let agentHandler message (self : NamedObject) loop (state : ThingState) = 
    actor { 
        match message with
        | Look -> 
            let objectNames = 
        ...
        | Enter(nameOfObject) -> 
            ...
            match findResult with
            | Some(no) -> 
                notifyContainer state.container.ref "{0} enters {1}" [ self.name.yellow; no.name.yellow ] 
                    [ self.ref ]
                no.ref <! AddContent(self)
                notify self.ref "You enter {0}" [ no.name.yellow ]
        ...
        | Put(nameOfTarget, nameOfContainer) -> 
            ...
            | Some(no1) -> 
                ...
                | Some(no2) -> 
                    ...
                    notify self.ref "You put {0} in {1}" [ no1.name.yellow; no2.name.yellow ]
